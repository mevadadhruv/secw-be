...
export const UserValidation = async(req:Express.Request,res:Express.Response,next:Express.NextFunction) => {
    const userSchema = Joi.object().keys({
        emailId : Joi.string().pattern(/^([\w-\.]+@([\w-]+\.)+[\w-]{2,4})?$/)
        .messages({
            "string.empty":"email can not be empty",
            "string.pattern.base":"Please enter the valid email address",
            "any.required":"Emailid is required"
        }).required(),
        password : Joi.string().pattern(/^(?=.*[!@#$%^&*_=+-]).{8,12}$/)
        .messages({
